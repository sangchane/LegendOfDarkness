...
## `map-source/` — 이식에 쓰는 맵 바이너리 (2026-09-12)

`5.99 서버팩` 의 맵 파일 중 **이식에 필요한 521개만** 팩 경로 그대로 들여온 것(8.45MB).
아카이브 전체 2,766개 중 19% 다 — 맵 807개가 파일 521개를 돌려 쓰기 때문이다
(혼돈의탑 71방이 한 파일, 네 마을 은행이 한 파일).

`server-packs/` 는 **텍스트만** 두는 곳이라 바이너리를 섞지 않으려고 여기 따로 두었다.
경로는 `maps.json` 의 `맵파일` 칸과 같아서 변환 없이 찾힌다.

**팩의 `너비`·`높이` 를 믿지 마라.** 807개 선언 중 12개가 파일 크기와 어긋난다.
대조표는 `plans/5.99-맵선언검증.tsv`.
