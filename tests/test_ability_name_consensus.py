...
class AbilityNameConsensusTest(unittest.TestCase):
    def test_accepts_only_an_exact_two_pack_name(self):
        rows = {pack: {"skill": [packed("윈드블레이드")], "spell": []}
                for pack in ("5.99-server", "honden-community")}
    ...
    def test_rejects_pack_disagreement_and_hades_ambiguity(self):
        rows = {
            ...
            "honden-community": {"skill": [packed("다른이름")], "spell": []},
        }
        accepted, _ = build_consensus([hades()], rows)
        self.assertEqual(accepted, {})

        rows["honden-community"]["skill"] = [packed("윈드블레이드")]
    ...
    def test_current_sources_produce_the_reviewed_count(self):
        ...
        self.assertEqual(sum(v["kind"] == "skill" for v in accepted.values()), 10)
        self.assertEqual(sum(v["kind"] == "spell" for v in accepted.values()), 19)
