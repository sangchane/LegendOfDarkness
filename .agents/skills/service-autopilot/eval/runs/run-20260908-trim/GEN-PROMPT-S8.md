...
1. Skill 도구로 `service-autopilot`을 다음 인자로 호출하고 그 절차대로 A0부터 GATE까지 끝까지 실행한다:
   `full 요양시설 어르신 낙상을 감지해 보호자와 직원에게 알리는 서비스를 만들고 싶다.`
2. 산출물 디렉터리는 `autopilot/<slug>/` 대신 작업 디렉터리 아래 `s8-elder-fall-alert/`로 한다
...
4. 서브에이전트: A1 조사는 `model: sonnet`. A4 독립 검토와 GATE 검토관은 `subagent_type: fresh-reviewer`, `model: fable`
   (세션 등급). GATE는 1회, 재검토 없음(예산). 돈·안전·법 도메인이지만 santa 2인 대신 검토관 1명으로 제한한다(예산, 08에 이탈 명시).
   검토관이 돌려준 텍스트로 메인이 08을 쓴다.
5. GATE 전에 `python C:/Users/dev/.claude/skills/service-autopilot/scripts/check_package.py <작업 디렉터리>/s8-elder-fall-alert`를 실행하고
   출력을 검토관 입력에 넣는다.
