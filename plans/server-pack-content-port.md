...
|---|---|---|---|---|---|
| 맵 | 807 | **797** | 선언이 파일과 어긋남 10 (아래 A) | 4 | `Map Templates Loaded: 801` |
...
### A. 선언이 파일과 어긋나는 것 11건 — 파일이 정답이다

**팩의 `너비`·`높이` 를 믿으면 안 된다.** 맵 파일을 실제로 재 보니 807개 선언 중 **12개가 어긋난다.**
칸은 `Cols` 마다 줄바꿈되므로 `Cols` 가 틀리면 맵이 뒤엉킨다 — 그래서 서버가 거른다.

| 맵 | 선언 | 파일 | 판정 |
|---|---|---|---|
| 흉가2층 @`서쪽대륙/lod10293` | 29×45 | **29×50** | **맞바꾸면 풀린다** ↓ |
| 흉가2층 @`서쪽대륙/lod10287` | 29×50 | 29×45 | 버린다 (그 파일은 흉가1층 것) |
| 집털 1-1 ~ 1-4 @`default/lod3713` | 25×25 | 50×50 | 보류 — 그 파일은 `해안가 3` 것 |
| 집털 1-5 @`default/lod3716` | 25×25 | 50×50 | 보류 — 그 파일은 `해안가 5` 것 |
| 결계대기실·결계2대기실 @`default/lod7000` | 16×16 | 700칸 | 보류 (14×50 / 20×35 / 25×28) |
| 노바방어구점 @`default/lod2650` | 15×15 | 144칸 | **12×12 이 유일** — 고칠 수 있다 |
| 아벨길드성-1층로비 @`길드성/lod3210` | 29×29 | 900칸 | 보류 (30×30 유력) |
| 아벨길드성-외곽 @`길드성/lod3209` | 63×43 | 2,925칸 | 보류 (45×65 / 39×75) |

`흉가2층` 둘은 **두 파일 사이에 값이 뒤바뀐 것**이다. `lod10293` 을 29×50 으로 고치면 붙고,
`lod10287` 쪽 선언은 버린다(그 파일은 29×45 로 `흉가1층` 이 이미 맞게 쓰고 있다).
`노바방어구점` 은 144칸이라 12×12 말고 답이 없다 — 이것도 고친다.
이것으로 **795 + 2 = 797.**

나머지 **10건은 보류**한다. 칸 수만으로는 가로·세로를 못 가린다 — 잘못 찍으면 맵이 뒤엉킨 채
멀쩡해 보인다. 답이 하나뿐인 것만 고쳤고, 나머지는 게임 안에서 눈으로 보고 정한다. 전체 표: `plans/5.99-맵선언검증.tsv`

남는 이름 중복은 둘: `연습장`(default/lod1956 · 노비스마을/lod4666),
`뤼케시온필드`(rucesion/lod505 · rucesionfield/lod505).
**이 이름들을 부르는 워프·젠은 0건**이라 개명해도 참조가 안 깨진다.
...
```
0 관문 ✅ 맵 521개 확보 ─┬── 3 전역 번호 ── 4 맵 ──┬── 5 워프 ── 5b 월드맵·문
                        │                         ├── 7 괴물 배치
...
## 0. 관문 — 맵 파일 521개  ✅ **통과 (2026-09-12)**

### 결과
`5.99 서버팩` 이 이 Mac 에 이미 풀려 있었다(`~/Downloads/5.99 서버팩`, 맵 2,766개, 한글 폴더명 정상).
필요한 **521개를 전부 찾았고 `data/map-source/5.99-server/` 에 팩 경로 그대로 들여왔다. 8.45MB.**
...
### 여기서 알게 된 것 — 파일이 정답이다
크기를 대조하다 **팩의 `너비`·`높이` 가 파일과 어긋나는 선언 12개**를 찾았다(위 A).
`lod3713.map` 은 실제 50×50 이라 `해안가 3` 이 맞고 `집털 1-1~1-4` 가 틀렸다.
**선언을 믿고 파일을 고르면 안 된다.** 전체 대조표: `plans/5.99-맵선언검증.tsv`

### 남은 일 (3단계에서 판정한다)
- `흉가2층` 을 `lod10293` @29×50 으로 고치고 `lod10287` 쪽 선언은 버린다 → **797** (`노바방어구점` 12×12 정정 포함)
- 나머지 9건은 보류. 칸 수만으로 가로·세로를 못 가린다

### 되돌리기
`data/map-source/` 를 지운다. 원본은 `~/Downloads/5.99 서버팩` 에 그대로 있다.
...
- 9갈래 전부 `--dry-run` 이 돌고, **"넣을 수"가 위 실측 표와 정확히 같다**
  (맵 797 · 워프 886 · 아이템 989 · 괴물 565 · NPC 84 · 기술 82 · 마법 71 · 월드맵 1 · 문 1)
- 못 넣는 것의 수와 이유도 표와 같다 (맵 10 · 워프 223 · 괴물 5 · NPC 95)
- 아직 **아무것도 쓰지 않는다**
...
1. `plans/5.99-맵번호표.tsv`: `출처<TAB>이름<TAB>새번호<TAB>맵파일경로<TAB>넣나(Y/N)<TAB>사유`
   ...
   - 새 번호는 **20000 부터**. **1~65535 만 쓸 수 있다** — 맵 번호는 전선에서 16비트다(`0x15`).
     10만번대를 쓰면 `100287` 이 클라이언트에 `34751` 로 온다(`& 0xFFFF`). 개수 검사도 서버
     로그도 이것을 못 잡는다 — 게임 안에서 들어가 봐야 드러난다.
   - **A 의 보류 10건은 `N` 으로 표시하고 사유를 적는다.** `흉가2층`·`노바방어구점` 은 고쳐서 `Y` 로.
...
### 완료 기준
- 전체 **807** · 넣는 것 **797** · 번호 유일 · 20000 미만 또는 65535 초과 **0** · 이름 겹침 **0** · 금지문자 **0**
...
### 할 일
1. `--kind maps --write` — `areas/<이름>.json` 797장 (`Id` 는 3단계 표에서, 파일명 소문자)
2. `maps/lod{번호}.map` 797개 — 원본을 번호마다 복사
3. **시험을 고친다** (공통 규칙 5):
...
ls sources/.../database/server/areas/*.json | wc -l    # 801
python3 -c "
...
### 완료 기준
- `Map Templates Loaded: 801` (팩 797 + 기존 4) · `Not loaded` **0건**
- **`areas/*.json` 파일 수 == 801 이고 `Id` 중복 0.**
...
### 되돌리기
`areas/`·`maps/` 에서 번호 20000 이상인 것과 그 이름의 json 을 지우고 재기동.
