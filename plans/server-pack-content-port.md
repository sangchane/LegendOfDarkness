...
|---|---|---|---|---|---|
| 맵 | 807 | **796** | 선언이 파일과 어긋남 11 (아래 A) | 4 | `Map Templates Loaded: 800` |
...
나머지 **11건은 보류**한다. 칸 수만으로는 가로·세로를 못 가린다 — 잘못 찍으면 맵이 뒤엉킨 채
멀쩡해 보인다. `노바방어구점`(12×12 이 유일)처럼 답이 하나뿐인 것만 고치고,
나머지는 게임 안에서 눈으로 보고 정한다. 전체 표: `plans/5.99-맵선언검증.tsv`
...
### 남은 일 (3단계에서 판정한다)
- `흉가2층` 을 `lod10293` @29×50 으로 고치고 `lod10287` 쪽 선언은 버린다 → **796**
- `노바방어구점` 을 12×12 로 고친다(답이 하나뿐이다)
...
- 9갈래 전부 `--dry-run` 이 돌고, **"넣을 수"가 위 실측 표와 정확히 같다**
  (맵 796 · 워프 886 · 아이템 989 · 괴물 565 · NPC 84 · 기술 82 · 마법 71 · 월드맵 1 · 문 1)
- 못 넣는 것의 수와 이유도 표와 같다 (맵 11 · 워프 223 · 괴물 5 · NPC 95)
- 아직 **아무것도 쓰지 않는다**
...
1. `plans/5.99-맵번호표.tsv`: `출처<TAB>이름<TAB>새번호<TAB>맵파일경로<TAB>넣나(Y/N)<TAB>사유`
   ...
   - **A 의 보류 11건은 `N` 으로 표시하고 사유를 적는다.** `흉가2층`·`노바방어구점` 은 고쳐서 `Y` 로.
...
### 완료 기준
- 전체 **807** · 넣는 것 **796** · 번호 유일 · 100000 미만 **0** · 이름 겹침 **0** · 금지문자 **0**
...
### 할 일
1. `--kind maps --write` — `areas/<이름>.json` 796장 (`Id` 는 3단계 표에서, 파일명 소문자)
2. `maps/lod{번호}.map` 796개 — 원본을 번호마다 복사
3. **시험을 고친다** (공통 규칙 5):
...
ls sources/.../database/server/areas/*.json | wc -l    # 800
python3 -c "
...
### 완료 기준
- `Map Templates Loaded: 800` (팩 796 + 기존 4) · `Not loaded` **0건**
- **`areas/*.json` 파일 수 == 800 이고 `Id` 중복 0.**
...
| 4단계에서 `areas/*.json` 수가 800 이 아니다 | 이름 충돌이 남았다. 3단계로 돌아간다 |
