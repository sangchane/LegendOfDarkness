...
|---|---|---|
| 월드 | ~~위쪽 400. 조작 영역과 겹치지 않는다~~ → **화면 전체, HUD가 그 위에 뜬다**(2026-09-18 사용자 — 조작 뒤 검은 공간이 3분의 1). 캐릭터는 위 줄과 기록 줄 사이 한가운데에 선다(`WorldView.FocusY`) | 화면 전체. HUD가 그 위에 뜬다 |
| 손가락 가림 | 캐릭터 둘레는 없다. 맵 아래쪽은 조작 밑으로 이어진다(걷는 동안 방향판이 흐려진다) | 아래 양쪽 모서리를 가린다 |
...
| 빠른 슬롯 | ~~가운데 아래 가로줄~~ → **공격 둘레 부채꼴**(2026-09-18 사용자 선택, 한 장 여섯 · 기술/마법 전환 · 다음 장). 상자 184×248 이라 방향판 옆 폭에 꼭 맞는다 | ~~오른쪽 세로줄~~ → 같은 부채꼴 |
...
### 가로 — 행간 0, 스크롤 없이 한 화면에

기준 800 x 360, 최악 640 x 360.

- 가로에서는 **행간 0**, 칸을 붙인다. 원작도 칸이 붙어 있다.
- **장비 18칸(칸 44)과 종이인형이 스크롤 없이 한 화면에 보인다.** 창은 화면 높이를 가득 쓰고,
  열려 있는 동안 오른쪽 위 줄을 덮는다.
- 머리 줄과 꼬리 줄은 칸들 옆 **기둥**으로 옮긴다 — 위에 탭과 `줍기`/`정렬`/`닫기`, 아래에 고른 것의 이름과
  `입기`/`벗기`/`버리기`. 그래서 고리가 창 높이를 다 쓴다.
- 칸 크기는 `GearLayout.CellThatFits` 가 정한다: 44 → 40 → 36 가운데 여섯 줄이 창에 다 들어가는 가장 큰 것.
  어느 것도 안 들어가면 36을 쓴다 — 고리가 다 들어가는 것보다 칸이 눌리는 것이 먼저다.
- 오른쪽 기둥의 폭은 고정 비율이 아니라 창이 잰 자기 최소 폭에서 거꾸로 잡는다(`SideColumn.LeftAnchor`).
