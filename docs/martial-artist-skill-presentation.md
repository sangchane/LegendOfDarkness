...
> 마구때리기 158→69 · 다라밀공 288→47 등. 동작(motion)·소리(sound)는
> 위 표의 5.99 값 그대로다. AGENTS.md 의 「팩은 3개가 일치할 때만」 규칙의 명시적 예외다.
> **노바 스크립트에 이펙트가 없으면 5.99 번호를 둔다**(0 으로 지우지 않는다) — 허공답보는 5.99 의 68 이 원작(사용자 2026-09-27).
> **이펙트 속도도 노바 값**(사용자 2026-09-27, 데빌크래셔 130→70 …) — 바꾼 줄 끝 `// 노바 이펙트(5.99: 쓴쪽, 대상, 속도 N)` 에
> 5.99 값을 남긴다. 무도가 템플릿에는 속도 칸이 없어 `MonkStrike` 가 100 을 보낸다(노바는 69·75 — 생성기 출력에 나온다).
