...
P0-11 일부 완료 — **캐릭터 이름으로 임의 파일 쓰기가 가능했다.** 이름이 네트워크에서 그대로 저장 경로에 붙어, `../escaped`는 `database/server/escaped.json`을, `../../escaped`는 `database/escaped.json`을 만들었다. 서버 프로세스가 쓸 수 있는 어느 위치든 이름 선택만으로 닿는다. load·save가 함께 지나가는 한 곳에서 경로를 해석하고, 캐릭터 폴더 **바로 아래**로 떨어지지 않는 이름은 거부한다. 남은 것: 이름 자체의 정책(길이·제어문자). CP949 한글 이름을 막지 않도록 별도로 정한다.
