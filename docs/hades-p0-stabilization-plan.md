...
P0-13 일부 완료. **소켓 정리는 이미 올바르다는 것이 측정으로 확인됐다** — 인증 전에 떠나는 연결 50회 뒤 서버가 붙잡고 있는 소켓(CLOSE_WAIT)은 0개다. 이 결과를 테스트로 고정했고, 소켓 열거가 실제로 이 서버의 연결을 본다는 대조군도 함께 넣었다.
...
남은 것: **handshake 시간 제한.** 잘린 본문을 보낸 연결은 느린 송신자와 형태로 구분되지 않아 시간 제한으로만 정리된다(P0-10에서 넘어온 항목).
