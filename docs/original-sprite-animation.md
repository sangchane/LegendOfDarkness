...
1. **겹치는 순서가 방향에 따라 바뀐다 — 5.99 클라이언트 실행 파일의 표 그대로다(2026-09-17 역어셈블).**
   `Legend.exe` 의 `0x69c200` 에 방향마다 15칸짜리 순서가 있고(부위 글자표 `"SBNLHUDHAWSCPEF"` 는 `0x86b614`),
   그리는 함수 `0x4e77e4` 가 그 줄을 그대로 따라 쌓는다. **동작(걷기·평타·기술)이나 칸 번호로는 바뀌지 않는다.**

   | 방향 | 아래 → 위 |
   |---|---|
   | 뒷모습(북·서) | 방패 · 몸 B · 바지 N · 신발 L · 갑옷 U · D · F · **무기 W** · 팔 A · 머리 H · E · P · 장신구 C |
   | 앞모습(동·남) | **무기 W** · F · 몸 B · 바지 N · 신발 L · 머리 H · 갑옷 U · D · 팔 A · E · P · **방패 S** · 장신구 C |

   - **앞모습에서는 무기가 맨 아래**라 몸·팔이 무기 쥔 손을 덮는다. 무기를 늘 위에 그리면 "무기 낀 손이 보여서는
     안 되는데 보인다"(사용자 지적). 뒷모습에서는 무기가 몸 위, 팔·머리 아래다.
   - `A` 는 갑옷 번호의 **팔**(`ma###`, 359종), `P` 는 무기 번호의 **몸 앞 무기 조각**(`mp###`, 71종 — 무도가
     손톱 117·127 도 있다), `E`·`F` 는 머리 번호의 앞·뒤 조각(약 90종). `D` 는 이름을 만들지만 파일이 없다.
     도포 `i` 는 이 클라이언트가 아예 부르지 않는다.
   - 좌우 뒤집기는 순서와 상관없다 — 다 쌓은 뒤 통째로 뒤집는다. `03` 파일 동작(손 들기·키스·손 흔들기)만 무기·P·방패를 뺀다.
   - 우리 코드: `Wardrobe.Rank`(순서) · `Actor.Face`(방향이 바뀔 때 다시 쌓기). 참고 클라이언트(dark-ages-ts)는
     방패만 옮겨서 이 규칙을 반쯤만 담았다.
...
| `01` | 10 | 10 | **서기 + 걷기** |
| `02` | 4 | 4 | **평타(기본 공격)** |
| `03` | 10 | 10 | **손 들기·키스·손 흔들기** (몸 동작 6·21·22 — 3.3절) |
| `b` | 20 | **14** | **성직자 스킬 동작** |
| `c` | 30 | **30** | **전사 스킬 동작** |
| `d` | 20 | **18** | **도가(무도) 스킬 동작** |
| `e` | 40 | **36** | **도적 스킬 동작** |
...
걷기·평타·손 들기·키스·손 흔들기는 `skill.tbl`에 없다. 클라이언트가 직접 들고 있는 값이다.

| 동작 | 몸 동작 번호 | 파일 | 등 구간 | 앞 구간 |
|---|---|---|---|---|
| 서기 | — | `01` | 0 | 5 |
| 걷기 | — | `01` | 1–4 | 6–9 |
| 평타 | 1 | `02` | 0–1 | 2–3 |
| 손 들기 | 6 | `03` | 0 | 1 |
| 키스 날리기 | 21 | `03` | 2–3 | 4–5 |
| 손 흔들기 | 22 | `03` | 6–7 | 8–9 |

**`03` 은 한 동작이 아니다 (2026-09-17, 2005 `Legend.exe` 역어셈블).** 전에는 "감정표현 0–4 / 5–9" 로 적었는데
틀렸다. 동작 번호 → 상태(0x4e1020: 1→3 · 6→4 · 21→7 · 22→8), 상태마다 시작 칸과 칸 수(0x4e3657..0x4e36c9 — 저장된
칸 수는 그림 수 + 1), 칸 고르기 `시작 + (칸 수 − 1) × 앞뒤 + 걸음 − 1`(0x4e23e8 · 0x4e24c5 · 0x4e2526, 걸음 0 은 `01`
의 서기 칸). **손 흔들기만 속도를 3 으로 나눠** 세 배 빨리 돈다. `03` 동작 동안은 방패(뒤·앞)·무기·무기 앞 조각을
그리지 않는다(0x4e78f4 → 0x4e84e0, 부위 0·9·10·12). 구현: `Lod.Mobile.Core.Art.BodyMotion.Of`, `Actor.MotionSheet`.
말풍선 감정표현(9~17 · 23~33)은 몸 그림이 아니라 여기 없다.
...
### 3.5 감정표현 — 몸이 아니라 머리 위 그림 (`emot01.epf`)

몸 동작 9~17 · 23~44 는 옷 파일이 아니라 `Legend.dat` 의 `emot01.epf` 를 머리 위에 겹친다(2026-09-17, 2005 `Legend.exe`
역어셈블 — 발췌 `data/disassembly/excerpts/original-2005-emote-4e1226.asm`). 서버는 클라이언트가 보낸 0~35 에 9 를
더해 보낸다(하데스 `Format1DHandler`). 이름은 Arbiter `BodyAnimation`(9 Smile … 44 Confused).

| 몸 동작 | 칸(0부터) | 칸마다 | 무엇 |
|---|---|---|---|
| 9~15 | 0~6 | 1.5초 | 얼굴 표정만(미소·울음·찡그림·윙크·놀람·메롱·흐뭇) |
| 16 · 17 | 7~8 · 9~10 | 1초 | 코 고는 방울 · 입 벌리기 |
| 23~26 | 11~14 | 1.5초 | 말풍선: 바위·가위·보·투덜 |
| 27 | 15~17 | 0.5초 | 말풍선 점 하나·둘·셋 |
| 28~41 | 18~31 | 1.5초 | 창백·홍조·하트·땀·음표·화남·… |
| 42 · 43 · 44 | 32~34 · 35~37 · 38~41 | 0.5초 | 눈물 · 불타는 눈 · 어지러움 |

- **표**는 실행 파일 안(0x869880)에 한 줄 64바이트로 있다 — 번호, 첫 칸(1부터, -1 이면 앞 줄에서 이어서), 칸 수(그림 수 + 1),
  칸마다 ms, 그리고 그 감정을 부르는 채팅 글("^^" · "ㅠㅠ" · "ㅇ_ㅇ" · "묵묵묵" · "빠직" …). 18·19 는 그림이 없는 줄이다.
- **앞모습(동·남)이고 하던 일이 없을 때만** 띄운다(0x4e1104). 떠 있는 동안은 바쁜 상태라 다른 동작을 무시한다.
- 칸이 여럿인 감정(16·17·27·42~44)은 칸을 넘기며 튼다고 보았다 — 칸을 넘기는 줄은 찾지 못했지만 표가 그만큼 칸을 건너뛰고
  그림이 차례를 이룬다(방울이 커졌다 작아지고, 점이 하나·둘·셋).
- **그리는 때:** 부위 순서표에서 첫 H(머리) 바로 앞(0x4e7ca7). 그래서 머리카락·투구가 맨머리 위를 덮는다.
