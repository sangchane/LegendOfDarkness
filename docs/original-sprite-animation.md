...
0~8이 `skill.tbl`의 NO와 같은 번호인지는 아직 확인하지 못했다 — 스킬 NO가 0~17이라 그대로는 안 맞는다.
...
부위 그림은 칸 안 좌표(`Left`·`Top`)를 **서로 공유한다.** 그래서 부위를 따로 뽑아도 **같은 칸 크기**
로만 뽑으면 그대로 겹쳐진다 — `dat-extract pose … <칸 크기>` 의 마지막 인자가 그것이고,
`build-client-assets.ps1` 이 전부 `47x83` 으로 뽑는다. 칸을 부위마다 자기 그림에 맞추면 모자가
머리에서 벗겨진다.

두 가지는 아직 안 된다.

- **무기(`w`)는 좌표를 공유하지 않는다.** `mw00101` 은 몸이 19–57 자리를 쓰는데 63 부터 그려져서,
  겹치면 사람 옆에 막대기가 따로 떠 있다. 무기만 다른 기준점을 쓰는 것으로 보이며, 이 서버에는
  무기를 든 사람이 없어 맞춰 볼 대상이 없다.
- **색(`HairColor`·`BootColor`)은 팔레트를 바꿔 끼우는 일**이라 그림을 겹치는 것만으로는 안 된다.
  참고 클라이언트는 조각마다 염색 셰이더를 건다(`paper-doll-container.ts` 의 `setDye`).
