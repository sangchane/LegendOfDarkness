...
| 무기 `mw001` · 장신구 `mc001` | 111x85 | 111x85 |

즉 **바탕 = max(머리말, 57x85)** 이다. 작은 조각은 머리말이 그림 크기를 담고 있어 그대로 믿으면
안 되고, 무기·장신구만 넓은 바탕(111x85)을 선언한다. 그래서 무기는 왼쪽으로 (111−57)/2 = **27** 만큼
당겨야 손에 들린다.
...
`dat-extract pose` 가 이 계산을 하고, 마지막 인자로 **모든 부위를 같은 칸(`80x88`)** 으로 뽑는다.
