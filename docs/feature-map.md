...
- **내 상태가 화면에 안 나온다.** 체력바가 고정값 `180 / 250`(`GS:439-448`)이고, 경험치·레벨·금화는 받기만 한다(`WC:869-916`).
...
| **성장** |||||
| 12 | 경험치·레벨업 | 돌아감 `HB/Types/Monster.cs:70-204` | 일부 — 수치만 받음 `WC:869-916`, 화면 없음 | `docs/monster-experience-rescale.md` | 원작 곡선 적용됨 |
...
| **NPC·경제** |||||
| 27 | NPC 대화·메뉴 | 돌아감 `GSH:1348-1438,1991-2012` | **없음** — 받기·답하기 `WC:413-468` 는 있는데 화면이 안 부른다 | `HB/Network/ServerFormats/ServerFormat2F.cs` | **모바일 최대 막힘** |
| 28 | 상점 | 돌아감 `S/Mundanes/shop1.cs:26-129` | 없음 | `HB/Network/ServerFormats/ServerFormat2F.cs:41-60` ItemSellData | 팔 물건 목록도 비었다(NEXT) |
...
| 38 | 금화 | 돌아감 — 버리기 `GSH:1009-1040`, 교환 `GSH:2272-2299` | 일부 — 버리기 `WC:534-542`, 표시 없음 | `TS:24` | |
| **함께 하기** |||||
...
| 0x3A | 대화 답(글·앞뒤) | `GSH:1440` | 돌아감 | 보냄(화면 없음) |
...
0x17·0x18·0x2C·0x2D 기술·마법 목록 · 0x19 소리 · 0x1A 몸 동작 · 0x29 이펙트 · 0x2F 대화창(화면 없음) · 0x33 사람 · 0x37·0x38 장비. 줄 번호는 `WC:239-415`.
...
1. **NPC 대화창(모바일)** — 서버·통신은 되고 화면만 없다. 이것 하나가 상점·은행·배우기·퀘스트·전직을 연다.
