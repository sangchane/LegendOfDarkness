...
|---|---|
| 로그인 → 캐릭터 확인 → 월드 입장 | 됨 |
| 계정·캐릭터 만들기 화면 | **안 됨** — 로그인 화면(`LoginScreen.cs`)은 계정·비밀번호 칸과 [로그인] 뿐이다. 머리 모양·성별·머리색을 고르는 화면이 없고, 알맹이(`HadesLoginClient`)도 로그인만 알아 계정·캐릭터 만들기를 보내는 길이 없다(서버는 `ClientFormat04` 로 그 세 바이트를 이미 받는다). 시험 계정은 `LoginFlow.TryCreateAccount` 가 고정값(머리 모양 1·성별 1·머리색 1)으로 만든다 |
| 지도·내 위치를 서버에서 받음 | 됨 |
| 월드맵(지도) | **됨(2026-09-19), 단 노비스마을에서는 실제로 안 열림** — 위 줄 [지도] 단추를 누르면 서버에 "열어 달라"고 말한다(`0xF0`, 우리가 만든 말 — 원작에는 없다). **몬스터가 없는 맵에서만 열린다**는 게 의도지만(싸우다 갇히지 않게), 노비스마을은 장식용 주민이 `Monster` 형으로 실려 있어 늘 거절당한다 — 아래 "확인한 것" 절. 뜬 창에서 한 곳을 고르면 그 맵으로 가고, [닫기]를 누르면 그냥 취소(`FieldChoice(0)`, 서버와 약속한 "갈 맵 0")만 보내 창을 거둔다 — 창에는 원래 닫기가 없었지만 사용자 결정으로 달았다. 창이 열려 있는 동안은 서버가 이 접속의 다른 패킷을 모두 버린다(`NetworkServer.cs:141`). 손 없이 확인: `--map` |
...
"어디로 갈까" 아래 24곳이 줄로 선다(`temuair.json`). **닫기가 없다** — 이 창이 열려 있는 동안 서버는
고르기 말고 이 접속의 패킷을 모두 버리므로(`NetworkServer.cs:141`), 닫아 봐야 걸음도 말도 닿지 않는다.
한 곳을 골라야 빠져나온다. 목록이 스크롤 칸(`ScrollContainer`)이라 한 화면에는 4곳 정도만 보이고
...
**노비스마을에서는 지도 단추가 실제로 안 열린다.** `--login <이름>:1234 --map --shot out.png
--shot-after 6` 으로 노비스마을에서 눌러 보면 창 대신 "이곳에서는 지도를 펼 수 없습니다." 만 온다 —
서버의 거절 문구 그대로다(`FormatF0Handler`, `GameServerHandlers.cs:1848`, "이 맵에 살아 있는 `Monster`
형이 하나라도 있으면 거절"). 노비스마을에는 장식용 주민 둘이 `templates/monsters/5.99/
노비스주민1·2@노비스마을.json` 로 놓여 있는데, 이름과 달리 **`Monster` 형으로 실려**(`MaximumHP`
21억·`ScriptName: "Common Monster"`) 이 거절에 걸린다. 서버를 막 띄운 순간에도 몇 초 안에 이미
서 있어(젠 간격이 짧다), 오래 켜 둔 서버에서는 피할 길이 없다. **격리 시험(`WorldMapMenuTests`)은
통과한다** — 그 젠이 서기 전에 시험이 끝나는 것뿐이라 우연이다. 화면 두 장은 그래서 몬스터 정의가
없는 수오미마을(캐릭터 저장 파일의 `CurrentMapId`·`X`·`Y` 를 서버를 세운 채 고쳐 옮김)에서 대신
찍었다 — 지도 단추 자체와 열기·닫기 배선은 거기서 그대로 확인된다. 노비스마을 쪽은 고치지 않았다 —
거절을 "살아 있는 아무 `Monster`" 가 아니라 **적대(hostile)** 로 좁히는 서버 쪽 수정이 필요하고,
이번 일의 범위 밖이다(`NEXT.md` 에 남김).
