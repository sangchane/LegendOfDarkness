...
| 걷기(예측 + 서버 정정) | 됨 |
| 다른 사람 표시·이동·사라짐 | 됨 |
| 사람마다 다른 옷 | **부분** — 서버가 말한 몸·머리·바지·신발·방패·갑옷·도포·무기·장신구를 겹쳐 그린다. 색은 아직 |
...
계정 두 개가 이미 있다 — `wren`(머리 3) / `friend`(머리 8), 둘 다 비밀번호 `test1234`
(머리 번호를 일부러 다르게 해 두었다 — 사람마다 다르게 그려지는지 눈으로 볼 수 있게)
