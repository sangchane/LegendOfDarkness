...
- **설정**: 설정 창의 「자동 사냥」 — 사냥 반경 4~20칸(슬라이더) · 회복 기술 1~99%(셀렉트 박스, 2026-09-26). 기기의 `user://autohunt.cfg` 한 줄(`12 50`).
...
- **앱**: 설정 창 [봇 부르기] → 봇이 오면 [봇 보내기](처음 이름은 [동료 부르기]). 결과는 서버 알림 한 줄("동료 companbot님이 왔습니다. (레벨 28)" ·
