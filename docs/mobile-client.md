...
| 인벤토리 | **됨** — 서버가 보낸 소지품(`0x0F`)을 목록으로. 열려 있는 동안 걷기·탭이 막힌다. 장착·사용·버리기는 범위 밖 |
| 괴물·상인 | **됨** — `0x07` 을 읽어 그리고, 탭해서 고를 수 있다. 그림은 `assets/actor/creature/mns###.png`, 없는 번호는 안 그린다 |
| 바닥 아이템 | **안 됨** — 우리 서버가 괴물 기록과 구별되지 않는 짧은 형식으로 쓴다 |
| 전투 | **부분** — `공격`이 서버에 닿고(`0x13`), 서버가 하는 말이 아래 줄에 뜨고(`0x0A`), 맞은 대상의 체력이 위 가운데에 뜬다(`0x13`, 백분율). 죽은 뒤 떨어지는 물건은 못 읽는다 |
| 대화 | **안 됨** — 시안만 있음 |
...
| `--strike` | (값 없음) | 자리를 잡으면 한 번 휘두른다. 잠시 뒤 괴물 체력과 서버가 한 말을 찍는다 |
