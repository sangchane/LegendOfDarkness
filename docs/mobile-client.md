...
- **앱**: 설정 창 [동료 부르기] → 동료가 오면 [동료 보내기]. 결과는 서버 알림 한 줄("동료 companbot님이 왔습니다. (레벨 28)" ·
...
- **알려진 한계**: 봇은 주인 버프가 걸렸는지 보지 못해 제 시계로만 다시 건다(봇이 새로 켜지면 한 번 "이미 걸려있습니다."가 갈 수 있다) · 해제(디나르콜리 등)는 쓰지 않는다 ·
  봇이 파티에 들어 사람의 경험치에 파티 몫(+5%)이 붙는다 · 봇이 쓰러지면(혼수) 멈출 뿐 스스로 일어나지 않는다 · 봇 여럿은 설정에 이름을 더하면 되지만 한 사람에 하나.
