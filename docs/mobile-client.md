...
| 위 줄 · 설정 · 창 하나(2026-09-26) | **위 줄 단추는 셋만** — [인벤토리] · [월드맵] · [설정]. [종료]는 설정 → **계정** 탭의 [종료] 로 옮겼다(판은 그대로 [로그아웃] · [게임 종료] · [취소], iOS 는 게임 종료가 로그아웃 대신). 세로 위 줄: 첫 줄 = 내 판(이름은 막대 위 한 줄) · 미니맵(남는 폭을 다 쓴다), 둘째 줄 = 고른 이 · 단추 셋. 가로: 내 판 · 고른 이 · 미니맵 · 단추 셋 한 줄(따로 있던 곳 이름 판은 미니맵 구석 글자로). **설정은 세 탭**(아이콘 탭 · 오른쪽 위 X): **자동**(포션 게이지 둘 · 자동 사냥 반경 · 회복 기술 % — 가로는 탭 안을 굴린다) / **봇**(봇 부르기·보내기) / **계정**(자동 로그인 끄기 · 종료). **큰 창은 한 번에 하나** — 인벤토리 · NPC 대화 · 기록(대화 창) · 월드맵 · 설정 · 길 찾기 · 봇 장비 중 하나를 열면 먼저 열린 것이 닫힌다. 모두 `GameScreen.SetWindow` 한 곳을 지나고, 규칙은 알맹이 `World/OneWindow.cs`(시험 `OneWindowTests`). NPC 대화는 닫힐 때 서버에 닫기를, 월드맵은 취소(맵 0)를 보낸다 — 전과 같다. 늦게 온 서버 닫기(0x30)는 지금 열린 다른 창을 닫지 않는다. 월드를 멈추는 것(걷기·탭 막힘)은 전처럼 인벤토리 · NPC 대화 · 기록 창뿐 |
...
| `--exit-menu` | (값 없음) | 자리를 잡고 90프레임 뒤 설정을 **계정 탭**으로 열고, 조금 뒤 그 **[종료]** 를 실제로 눌러(`EmitSignal`) 고르는 판([로그아웃] · [게임 종료] · [취소])을 띄운다 — 위 줄의 [종료]는 2026-09-26 설정 안으로 옮겼다. 판은 화면 가운데 가로로, [종료] 아래에. 판 밖을 누르면 닫힌다. **iOS 는 앱이 스스로 꺼질 수 없어(Godot `Quit` 이 아무것도 안 한다) [게임 종료] 가 로그아웃으로 대신한다** |
...
- **2단계(2026-09-26) — 이름 [봇 부르기]/[봇 보내기], 알림도 "봇 …"**
  ...
  - **앱**: 왼쪽 위 줄 아래 **봇 칸**(이름 + 체력 막대 굵게 + 마력 막대 얇게, 파티 목록에서는 봇을 뺀다) → 누르면 **봇 장비창**(`BotGearPanel` — 장비 고리 `GearGrid` 재사용,
