...
| `--percent-open` | `health`·`mana`·`heal` | `--settings` 와 같이 주면, 설정 창이 뜨고 잠시 뒤 그 셀렉트 박스를 스스로 눌러 1~99 목록을 열어 본다 |
...
| `--auto-hunt` | (값 없음) | 게임에 들어가 자리를 잡으면(약 2초) [자동] 단추를 스스로 눌러 자동 사냥을 켠다(`GREYBOX_AUTOHUNT 켬`·`멈춤 …`). 손 없이 확인할 때 — 아래 「자동 사냥」 |
...
게임 화면 위 줄의 **[자동]** 단추로 켜고 끈다. 켜지면 단추가 강조색(`Greybox.Commit`)으로 채워지고, 사람이 잠시
조작 중이면 흐려진다. 판단은 알맹이 `World/AutoHunt.cs`(시험 `AutoHuntTests` 23개), 앱은 그 결정을 걸음·평타·기술로
옮기기만 한다(`WorldView.AutoHuntTick`). 한 걸음이 끝난 뒤에만 묻기 때문에 걷기 빠르기는 수동과 같다(`StepSeconds` 0.4초 —
서버 걷기 제한 안).
...
- **사람이 손대면**: 방향판을 누르면 누르는 동안과 뗀 뒤 3초는 손에 맡기고, **선 자리가 새 중심**이 된다(사냥터를 옮긴 것).
  공격·기술 단추를 누르면 3초 쉬고(중심 그대로) 다시 돈다. 끄는 것은 [자동]을 다시 누를 때뿐 — 싸우다 한 번 친 것으로
  꺼져 버리면 다시 켜는 게 번거롭다. 길 찾기(길 창)로 걷는 동안에도 쉰다.
