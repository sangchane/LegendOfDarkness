...
| 인벤토리 | **됨** — `장비`·`소지품` 두 탭. 소지품은 원작식 아이콘 격자(입기·버리기·정렬), 장비는 원작 신형 장비창의 고리 배치 18자리에 종이인형(`docs/original-equipment-window.md`). 열려 있는 동안 걷기·탭이 막힌다. **벗기는 아직 안 됨** |
| 괴물·상인 | **됨** — `0x07` 을 읽어 그리고, 탭해서 고를 수 있다. 그림은 `assets/actor/creature/mns###.png`, 없는 번호는 안 그린다 |
| 바닥 아이템 | **됨** — 서버가 다른 것과 같은 형식으로 쓰게 고쳤다(fork). 그림은 없어서 자리 표식으로 그린다 — 아이콘 아카이브에서 아직 아무것도 안 뽑았다 |
