...
**실제로 서는 마릿수는 `SpawnMax` 그대로가 아니다** — `MonolithComponent` 한 곳이 정한다:
`round(SpawnMax × √(칸수 ÷ 400) × 0.7)`, 그리고 정의마다 칸수 ÷ 40 을 넘지 않는다. √ 는 20x20 방 기준 값을 넓은
맵에 맞추는 것(2026-09-18), × 0.7 은 아이폰으로 돌아본 사용자가 "30% 줄여 달라"고 한 것이다(2026-09-24 사용자 결정).
전체 합은 16,107 → 11,820(−27%, 반올림과 상한 때문에 30% 보다 조금 덜 준다). 노비스평원A 정의 하나(15)는 37 → 26,
우드랜드1-1 의 1마리 정의는 3 → 2. **경험치 0 인 정의(노비스마을의 노비스주민1·2)는 꾸밈이라 늘리지 않는다** — 5.99
`Npc_Spawn` 대로 한 명씩(전에는 3명씩). 시험 `SpawnCountTests`.

**2026-09-26 — 사냥감은 한 번 더 2/3, 다시 서는 간격은 1/3**(사용자 "젠 되는 숫자는 좀 줄이고, 리젠을 몬스터가 끊기지 않게").
경험치가 있고 `SpawnMax` 가 2 이상인 정의만: 마릿수 `round(SpawnMax × √배수 × 0.7 × 2/3)`(최소 1), 간격 `SpawnRate ÷ 배수 ÷ 3`.
희귀 단독개체(`SpawnMax` 1 — 사슴·보스)와 꾸밈(경험치 0)은 그대로다. 모든 맵에 같은 식이다(99 이후 맵은 아직 이어지지 않았다).
**멈추는 버그는 없었다** — 클라우드 기록에 젠 예외가 없고, 격리 서버에서 2분 쉬지 않고 잡아도 곁이 4초 넘게 빈 적이 없다.
느낀 까닭은 속도다: 정의 하나는 간격마다 **한 마리**만 세우고 그것도 맵 전체 아무 데나 세운다. 포테의숲6존은 1분에 12마리,
노비스평원A 는 60마리가 한계였다(주인 + 동료 봇이 함께 잡으면 비어 간다). 이제 36 · 180. 순회가 1초라 간격은 1초 아래로
줄지 않는다. 시험 `RespawnTests`(코앞 한 칸에 세운 노비스 정의 — 잡은 자리가 4초 안에 다시 찬다; 고치기 전 4.4초).

**맵을 옮기면 지난 맵 괴물을 화면에서 거둔다(2026-09-24).** 사용자가 본 "마을의 좀비 괴물 · 맵 밖에 선 괴물"은
서버에 있는 괴물이 아니라 **지난 사냥터 괴물의 그림**이었다. 서버는 맵을 옮기거나 같은 맵에서 순간이동하면
(`RefreshMap`) 제 쪽 시야를 비우고 0x15 를 보내는데, 보여 주던 것에 0x0E 를 보내지 않았다 — 원작 클라이언트는 0x15 에
화면을 비우지만 모바일은 비우지 않아, 움직이지도 죽지도 않는 괴물이 마을 위에(마을이 더 작으면 맵 밖에) 남았다.
이제 시야를 비우기 전에 하나하나 0x0E 로 거둔다. 시험 `SpawnCountTests.Leaving_a_map_takes_its_monsters_off_the_screen`
(고치기 전: 평원에서 본 2마리가 노비스마을 화면에 남음).
