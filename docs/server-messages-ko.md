...
## 팩 NPC 대사 속 영어 낱말 (2026-09-25)

5.99 팩 원문 자체가 한국어 문장 안에 영어 낱말을 그대로 남긴 자리 9곳. `Pack599/Npcs/*.cs` 는
`scripts/build-pack-npcs.py` 가 팩 원본에서 그대로 옮겨 쓰는 생성 파일이라(손으로 고치면 다시 돌릴 때
지워진다) 그 스크립트의 `TEXT_PATCH` 표에 고침을 적어 두었다 — 같은 팩 안에서 같은 낱말의 한국어
표기(`Npc_Warp.txt` 137줄 "3천골드")나 우리가 이미 옮긴 NPC(미용사·유령하녀의 "다음을 누르시면")가
있어 그대로 따랐다. 대표로 개인던전입장도우미·미용사 둘은 격리 서버로 열어 실제 창에 영어가 없음을
시험한다(`NpcDialogueTests.Ported_npc_dialogue_has_no_leftover_english_word`).

| 영어 | 한국어 | 근거 | NPC (파일) |
|---|---|---|---|
| `요금은 1500Gold입니다.` | `요금은 1500골드입니다.` | `Npc_Warp.txt` 273줄 | 개인던전입장도우미 |
| `오렌마을(5000Gold)` | `오렌마을(5000골드)` | `Npc_Warp.txt` 295줄 | 애교 |
| `요금은 1만Gold라네.` | `요금은 1만골드라네.` | `Npc_Warp.txt` 314줄 | 워프할아버지 |
| `요금은 30만Gold라네.` | `요금은 30만골드라네.` | `Npc_Warp.txt` 584줄 | 타바리마을이동 |
| `(요금5000GOLD)` | `(요금5000골드)` | `Npc_Script.txt` 157줄 | 미용사 |
| `...가시겠습니까?Next를 누르면...` | `...가시겠습니까? 다음을 누르면...` | `Npc_Script.txt` 408줄 | 선진 |
| `Next를 누르시면...` | `다음을 누르시면...` | `Npc_Script.txt` 416줄 | 선진 |
| `NEXT를 누르시면 마을로...` | `다음을 누르시면 마을로...` | `Npc_Script.txt` 232줄 | 리프트도우미 |
| `...틀립니다. ex)모든 몬스터 처치...` | `...틀립니다. 예)모든 몬스터 처치...` | `Npc_Script.txt` 73줄 | 멜로린 |
| `생셋 or 암셋은 필수!` | `생셋 또는 암셋은 필수!` | `Npc_Making.txt` 92줄 | 신의대장장이1 |

**손대지 않은 것.** `EG`(달인·멜로린의 화폐 단위) · `PK`(전투 스킬·스펠의 안내문) · `NPC`(신의대장장이1
의 자기소개) · `GP`(멜로린·초보자도우미2 의 배고픔 수치) — 팩 자신도 여기저기서 그대로 쓰는 게임
줄임말이라 `HP`·`MP`처럼 이미 굳은 표기로 본다. 무도가 기술·스펠(`Pack599/Skills`·`Pack599/Spells`,
"PK맵에서는..." 등)은 다른 에이전트가 옮기는 중이라 손대지 않았다.
