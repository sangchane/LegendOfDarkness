...
| 체력·마력 아이콘 | **채워지는 막대**(`GameScreen.Gauge`, 세로 10 · 이름+숫자 곁들임) — 2026-09-18엔 18px 원 하나였다가 2026-09-25 사용자 지시로 되돌렸다 | 숫자만으로는 한눈에 안 찬다는 지시. 위키프레임(`docs/mobile-test-v1-wireframes.md`) 도 원래 "막대와 숫자를 함께"였다 |
| 치수 | 틀 5 · 속 12 · 틈 8 · 누르는 곳 44 이상 · 모서리 12(속에만) | 돌 틀은 각져야 원작처럼 보인다 |
| 위 판과 머리 위, 둘 다 그린다(2026-09-25) | 머리 위 = 백분율 막대(맞을 때만, `HealthBar.cs`) · 위 판 = 채워지는 막대 + 정확한 숫자(늘) | 전에는 "두 번 그리지 않는다"며 위 판을 숫자만 남겼으나(`2faede52`), 사용자가 다시 막대를 요구했다 — 위 판은 항상, 머리 위는 맞을 때만이라 겹치는 자리·시간이 없다 |
