...
- **[지금/화면]** **`벗기` 가 없다.** 장비 탭에서 걸친 것을 고르면 이름만 뜬다 — 클라이언트가 벗는 명령을
  모른다(`WorldClient` 에 `UseAsync`·`DropAsync`·`MoveAsync` 뿐). Arbiter 에 `ClientUnequipItemMessage`
  가 있으니 Hades 가 받는 번호를 찾아 잇는다
- **[다음/화면]** **바닥에 떨어진 것을 줍는 길이 없다.** 그림은 나오지만 주울 수 없다 — 원작은 그 칸을 밟거나 누르면 줍는다. 서버 쪽 명령을 찾아 소지품까지 이어야 전투 한 바퀴가 닫힌다
