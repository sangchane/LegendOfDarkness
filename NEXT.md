...
- **[지금/클라이언트]** **아이패드로 로그인할 계정이 없다.** 주소는 붙었다 — 실기기 빌드는 `Main.MobileTestServer`(지금 `192.168.0.8:2610`)를 보고, Mac 의 서버는 돌고 있다. 그런데 `database/server/aislings` 가 비어 있고, 클라이언트에는 회원가입이 없으며(시안 3절 범위 제외) `Lod.Mobile.Core` 에도 계정 생성이 없다. 셋 중 하나다 — ① Core 에 계정 생성(`0x02`·`0x04`)을 넣는다(시험 하네스 `tests/hades-characterization/LoginFlow.cs` 에 절차가 있다) ② 윈도우 PC 의 aisling JSON 을 옮겨 온다 ③ 원본 7.18 클라이언트로 만든다(Mac 에는 없다)
