...
- **[지금/서버]** **돈 합산을 실제로 확인해야 한다.** 고쳐서 fork 에 올렸다(`fix: stack money dropped where money already lies`) — 같은 칸의 돈을 걷어 합친 금액으로 다시 놓는다. 서버는 깨끗하게 뜨지만 **쌓이는 것 자체는 아직 눈으로 못 봤다**: 같은 칸에 두 번 떨어뜨릴 수 있는 판이 필요하다(괴물 둘을 같은 자리에서 잡거나, 떨어뜨리기·줍기를 쓴다)
