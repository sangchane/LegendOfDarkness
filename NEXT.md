...
- **[지금/자산]** **아이템 아이콘.** 바닥에 떨어진 것도 인벤토리에 든 것도 지금은 글자와 자리 표식뿐이다. **어디 있는지는 찾았다** — `ia.dat` 의 `stc#####.hpf` 19,664장(폭 고정, 머리말 8바이트, 앞 네 바이트가 `FF02AA55` 면 압축 — `da-lib` 의 `HpfFile`). 남은 것 둘: ① 서버의 `DisplayImage`(예: 32882) → 파일 번호 대응(`DADataViewer/ItemsForm` 은 `(파일-1)*266 + 칸 + 1` 로 센다), ② 팔레트 고르기(`stcpal.tbl` + `stc####.pal` 189장)
