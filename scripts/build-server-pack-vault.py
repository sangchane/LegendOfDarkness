...
def build(pack):
    ...
    for cat, key in CATS:
        ...
        for name, entries in merged.items():
            ...
            elif cat == "NPC" and npc_maps.get(name):
                body += [f"## 서 있는 맵 ({len(npc_maps[name])})",
                         ", ".join(maybe("맵", m) for m in sorted(npc_maps[name])), ""]
            elif cat == "아이템" and item_shops.get(name):
                body += [f"## 파는 곳 ({len(item_shops[name])})",
                         ", ".join(maybe("상점", s) for s in sorted(item_shops[name])), ""]
            elif cat == "상점":
