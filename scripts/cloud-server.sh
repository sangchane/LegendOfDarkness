...
upload() {
    ...
    rsync -az --delete -e "ssh -i $KEY" --exclude 'Hades_*.txt' --exclude 'LoruleConfig.json' --exclude 'MServerTable.xml' \
        "$FORK/Staging/net9.0/" "$HOST:$REMOTE/Staging/net9.0/"
    rsync -az -e "ssh -i $KEY" "$conf/" "$HOST:$REMOTE/Staging/net9.0/"
    rsync -az -e "ssh -i $KEY" --exclude 'aislings/' "$FORK/database/server" "$FORK/database/assets" "$HOST:$REMOTE/database/"
    rm -rf "$conf"
}
...
( crontab -l 2>/dev/null | grep -v lod-backup; echo '0 4 * * * tar -czf /home/ubuntu/backups/aislings-$(date +\%Y\%m\%d-\%H\%M).tar.gz -C /home/ubuntu/lod/database/server aislings && ls -1t /home/ubuntu/backups/aislings-*.tar.gz | tail -n +31 | xargs -r rm -- # lod-backup' ) | crontab -
...
SH
    ...
    upload
    # 처음 한 번만 맥의 캐릭터를 올린다.
    rsync -az -e "ssh -i $KEY" "$FORK/database/server/aislings" "$HOST:$REMOTE/database/server/"
    restart
}
...
case "${1:-status}" in
    ...
    logs) logs "${2:-40}" ;;
    backup) backup ;;
    *) echo "쓸 수 있는 것: setup deploy restart status logs backup"; exit 2 ;;
esac
