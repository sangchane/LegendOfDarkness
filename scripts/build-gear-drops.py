...
#
# **접미사(사람이름) 반지는 11~12레벨 한 층뿐이다.** 하데스표 496종(`data/pack-compare/한글이름-검토.tsv`)
# 을 다 훑어도 이 반지·귀걸이 계열은 5~16레벨 보석 등급(호안석·산호·고루…)이 전부고 21·41·71레벨에
# 해당하는 등급이 없다 — `scripts/build-suffix-gear-ko.py` 로 되살렸다. 그래서 접미사 반지는
# 우드랜드3-4(11레벨 대) 한 곳에만 쓰고, 그 위 세 사냥터는 이미 4원소(화염·바다·바람·대지)로 층이 있는
# **공격 속성** 장비를 그 층의 레벨에 맞춰 쓴다(11 → 룬스톤목걸이, 51 → 크리스탈목걸이, 71 → 홀디트링
# — 21·41 레벨 그대로는 없어 위아래 중 있는 층으로 채웠다. 세일라링은 원소마다 레벨·값이 서로 달라
# 들쭉날쭉해 보여 쓰지 않았다).
TIERS = [
    (
        [20023, 20024],
        "우드랜드3-1·4-1",
        "21~99",
        # 11레벨 대 접미사 반지·목걸이 한 벌 — 로오·칸은 5.99 팩 자기 것, 나머지 다섯은 되살린 것(12레벨).
    ...
    (
        ...
        "포테의숲1~6존",
        "21~52",
        # 4원소 룬스톤목걸이(11레벨) — 21레벨에 맞는 층이 없어 바로 아래 층을 쓴다.
    ...
    (
        ...
        "우드랜드5-1·6-1",
        "51~99",
        # 4원소 크리스탈목걸이(51레벨) — 41레벨에 맞는 층이 없어 바로 위 층을 쓴다.
